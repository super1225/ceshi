...
import com.example.demo3.dao.bean.Blog_account;
...
public interface Blog_accountRepository {
    
    //根据用户id确认用户信息-ok
    @Select("select * from blog_account where id = #{id}")
    Blog_account getBlogAccountById(@Param("id") int id);

    @Select("select * from blog_account where username = #{username}")
    Blog_account getBlogAccountByusername(@Param("username") String username);

    @Select("select * from blog_account where account = #{account}")
    Blog_account getBlogAccountByaccount(@Param("account") String account);
